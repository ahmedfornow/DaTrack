...
**Current:** the rebuild in `app/` — Vite, React, TypeScript, Tailwind, Vitest. Merging
to `main` builds, tests, and deploys it; there is no manual publish step. See
`docs/REDESIGN_BRIEF.md` for the design direction.

**Legacy:** one 1,855-line `index.html` — inline CSS, inline JS, Supabase + Chart.js
from CDN, no build step. Still published at `/legacy/` as the fallback, and still the
...
without also updating the repo copy, because the build asserts the two are
byte-identical and will abort the deploy if they drift.
